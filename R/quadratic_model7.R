...
quadratic.fn = function(mkt.id, rf_code, run.date,alltraffic.df) {
  ...
  vl=which(!(is.na(traffic.df$CID))) #Anirban
  cid=traffic.df$CID[vl[length(vl)]] #Anirbna
#Anirban #commented  cid = traffic.df$CID[1]
  ...
  if (nrow(traffic.df) > 0) { #10: Only non-penny bids
    ...
    #Anirban Should we do this for TRNETREV
   traffic.df$NETREV_ACT = traffic.df$TRREVENUE - traffic.df$COST #Anirban
    ...
    if (sum(traffic.df$CLICKS) > 0) {
      ...
      #Anirban Should traffic.df$TRNETREV be adjusted as well?
      ########## Week day adjustment ######################################
      ...
      if (nrow(traffic.df[traffic.df$NETREV > 0,]) > 0) { #6: Atleast some rows where netrev > 0
   	...
   	colnames(traffic.df) <- c("MAX_CPC", "IMPR", "CLICKS", "NETREV_new", "NETREV", "DTE", "MKT_ID", "RF_CODE", "CID", "MAX_CPC1", "CLICKS_old", "COST", "REVENUE", "IMPR_old", "TRREVENUE","NETREV_ACT","TRNETREV","NETREV_old", "WDAY")		
		print(traffic.df)
        ...
        if (length(unique(traffic.df$MAX_CPC)) >=2) {
          ...
          pldf=traffic.df
          pldf$pred_netrev=predict(netrev.mdl,newdata=data.frame(MAX_CPC=pldf$MAX_CPC),type="response")
          par(mfrow=c(2,2))
          print(plot(y=pldf$pred_netrev,x=pldf$MAX_CPC,type="l"))
          print(plot(y=pldf$NETREV_ACT,x=pldf$MAX_CPC,col="red",type="l"))
          print(plot(y=pldf$NETREV,x=pldf$MAX_CPC,col="blue",type="l"))
          print(plot(y=pldf$TRNETREV,x=pldf$MAX_CPC,col="green",type="l"))
